...
const basefile = path.join(__dirname, '..', 'code', 'skeleton.h')
...
const versionGet = () => {
    ...
    const major = data.match(/SKEL_VERSION_MAJOR\s+([0-9]+)\n/)[1]
    const minor = data.match(/SKEL_VERSION_MINOR\s+([0-9]+)\n/)[1]
    const patch = data.match(/SKEL_VERSION_PATCH\s+([0-9]+)\n/)[1]
    const pre   = data.match(/SKEL_VERSION_PRE\s+\"([\.a-z0-9]+)\"\n/)
...
const versionSet = (version) => {
    ...
    data = data.replace(/SKEL_VERSION_MAJOR\s+([0-9]+)\n/, `SKEL_VERSION_MAJOR ${major}\n`)
    data = data.replace(/SKEL_VERSION_MINOR\s+([0-9]+)\n/, `SKEL_VERSION_MINOR ${minor}\n`)
    data = data.replace(/SKEL_VERSION_PATCH\s+([0-9]+)\n/, `SKEL_VERSION_PATCH ${patch}\n`)
    data = data.replace(/SKEL_VERSION_PRE\s+\"([\.0-9a-z]+)\"\n/, `SKEL_VERSION_PRE "${pre}"\n`)
...
const embedIncludes = (print) => {
    ...
    else       fs.writeFileSync(path.join(workdir, 'skeleton.h'), code)
}
...
class Bumper extends Plugin {
    ...
    afterRelease() {
        if (fs.existsSync(path.join(workdir, 'skeleton.h'))) {
            fs.unlinkSync(path.join(workdir, 'skeleton.h'))
        }
    }
}
